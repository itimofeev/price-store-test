...
func TestStore(t *testing.T) {
	store := New("mongodb://root:example@localhost:27017/db?connect=direct&authSource=admin&authMechanism=SCRAM-SHA-256")
	...
	t.Run("save new product", func(t *testing.T) {
		...
		require.EqualValues(t, 1, product1.UpdateCount)
	})
	...
	t.Run("update product with same name, increase update count", func(t *testing.T) {
		...
		require.EqualValues(t, 2, product2.UpdateCount)
